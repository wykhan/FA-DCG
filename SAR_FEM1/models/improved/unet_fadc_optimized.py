...
class UNetWithOptimizedFADCG(UNet):
    """U-Net + FA-DCG V1.1 with original bottleneck placement and vectorized block."""

    def __init__(self, in_channels=1, num_classes=1, features=[64, 128, 256, 512]):
        super().__init__(in_channels, num_classes, features)
        self.light_fadc = VectorizedLightFADC(features[-1] * 2, kernel_size=3)
...
def test():
    ...
    print(f"U-Net + Optimized FA-DCG: {params:.2f}M")
