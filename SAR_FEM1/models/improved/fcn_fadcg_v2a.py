...
class FCNWithFADCGV2a(FCN):
    """FCN + FA-DCG V2a with V1.1-compatible integration path."""
...
def test():
    ...
    model = FCNWithFADCGV2a(in_channels=1, num_classes=1)
    out = model(x)
    params = sum(p.numel() for p in model.parameters()) / 1e6
    print(f"FCN + FA-DCG V2a: {params:.2f}M")
