...
class UNetWithFADCGV2d(UNet):
    """U-Net + FA-DCG V2d with V1.1-compatible bottleneck placement."""

    def __init__(self, in_channels=1, num_classes=1, features=[64, 128, 256, 512]):
        super().__init__(in_channels, num_classes, features)
        self.light_fadc = FADCGV2d(features[-1] * 2, kernel_size=3)
...
def test():
    ...
    model = UNetWithFADCGV2d(in_channels=1, num_classes=1)
    out = model(x)
    params = sum(p.numel() for p in model.parameters()) / 1e6
    print(f"U-Net + FA-DCG V2d: {params:.2f}M")
