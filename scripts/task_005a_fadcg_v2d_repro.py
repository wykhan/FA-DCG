...
MODEL_FACTORIES = {
    "FCN + FA-DCG V2d": FCNWithFADCGV2d,
    "U-Net + FA-DCG V2d": UNetWithFADCGV2d,
}

BENCHMARK_FACTORIES = {
    "FCN + FA-DCG V1.1": ("v1.1", FCNWithOptimizedFADCG),
    "FCN + FA-DCG V2d": ("v2d", FCNWithFADCGV2d),
    "U-Net + FA-DCG V1.1": ("v1.1", UNetWithOptimizedFADCG),
    "U-Net + FA-DCG V2d": ("v2d", UNetWithFADCGV2d),
}
...
def collect_diagnostics(model: nn.Module, images: torch.Tensor):
    modules = [module for module in model.modules() if isinstance(module, FADCGV2d)]
    handles = []
...
def summarize(rows):
    ...
    for method in MODEL_FACTORIES:
        ...
        if delta_miou > 0:
            ...
            status = "v2d_comparable"
        elif delta_time > ref["time_ms"] * 1.5:
            status = "v2d_too_slow"
        else:
            status = "v2d_underperforming"
        summary.append(
            {
                "method": method,
                "implementation": "v2d",
...
def write_report(out_dir: Path, summary, speed_rows, args):
    ...
    (out_dir / "task_005a_report.md").write_text("\n".join(report) + "\n", encoding="utf-8")


def main():
    args = parse_args()
    if args.output_root is None:
        args.output_root = str(REPO_ROOT / "exp" / "task_005a_fadcg_v2d_repro" / datetime.now().strftime("%Y%m%d_%H%M%S"))
    out_dir = Path(args.output_root)
    (out_dir / "logs").mkdir(parents=True, exist_ok=True)
    (out_dir / "checkpoints").mkdir(parents=True, exist_ok=True)
    (out_dir / "figures" / "fadcg_v2_diagnostics").mkdir(parents=True, exist_ok=True)
    write_environment(out_dir)
    test_output = run_cmd([sys.executable, "scripts/task_005a_test_fadcg_v2d.py"])
    write_audit(out_dir, test_output)
    (out_dir / "run_commands.sh").write_text(
        "#!/usr/bin/env bash\n"
        "set -euo pipefail\n"
        f"python scripts/task_005a_fadcg_v2d_repro.py --output-root {out_dir} "
        ...
        for method in args.models:
            for seed in args.seeds:
                ...
                row = {
                    ...
                    "notes": "validation split used; Speckle-Robust Frequency Gate",
                }
                rows.append(row)
                write_csv(
                    per_seed_path,
