...
MODEL_FACTORIES = {
    "FCN + FADC": FCNWithFADCAligned,
    "U-Net + FADC": UNetWithFADCAligned,
}

BENCHMARK_FACTORIES = {
    "FCN + FA-DCG V1.1": ("v1.1", FCNWithOptimizedFADCG),
    "FCN + FADC": ("fadc_aligned", FCNWithFADCAligned),
    "U-Net + FA-DCG V1.1": ("v1.1", UNetWithOptimizedFADCG),
    "U-Net + FADC": ("fadc_aligned", UNetWithFADCAligned),
}
...
def collect_diagnostics(model: nn.Module, images: torch.Tensor):
    for module in model.modules():
        if isinstance(module, FADCAligned):
            channels = module.channels
            sample = images.new_zeros((min(images.size(0), 2), channels, 32, 32))
            module.get_diagnostics(sample)
            return {key: tensor_to_value(value) for key, value in module.last_diagnostics.items()}
    return {}
...
def source_check():
    ...
    for path in paths:
        ...
        if "mmcv" in text or "mmseg" in text:
...
def summarize(rows):
    ...
    for method in MODEL_FACTORIES:
        ...
        if delta_miou > 0:
            status = "aligned_and_better"
        elif abs(delta_miou) <= 0.005:
            status = "aligned_and_comparable"
        elif delta_time > ref["time_ms"]:
            status = "aligned_but_slower"
        else:
            status = "underperforming"
        summary.append(
            {
                "method": method,
                "implementation": "fadc_aligned",
...
def write_audit(out_dir: Path, test_output: str, source_violations):
    text = [
        ...
        f"- Source check violations: {source_violations or 'none'}.",
        "",
...
def write_report(out_dir: Path, summary, speed_rows, args):
    ...
    (out_dir / "task_003_report.md").write_text("\n".join(report) + "\n", encoding="utf-8")


def main():
    args = parse_args()
    if args.output_root is None:
        args.output_root = str(REPO_ROOT / "exp" / "task_003_fadc_aligned_repro" / datetime.now().strftime("%Y%m%d_%H%M%S"))
    out_dir = Path(args.output_root)
    (out_dir / "logs").mkdir(parents=True, exist_ok=True)
    (out_dir / "checkpoints").mkdir(parents=True, exist_ok=True)
    (out_dir / "figures" / "fadc_diagnostics").mkdir(parents=True, exist_ok=True)
    write_environment(out_dir)
    source_violations = source_check()
    test_output = run_cmd([sys.executable, "scripts/task_003_test_fadc_aligned.py"])
    write_audit(out_dir, test_output, source_violations)

    (out_dir / "run_commands.sh").write_text(
        "#!/usr/bin/env bash\n"
        "set -euo pipefail\n"
        f"python scripts/task_003_fadc_aligned_repro.py --output-root {out_dir} "
        ...
        for method in args.models:
            for seed in args.seeds:
                ...
                row = {
                    ...
                    "notes": "validation split used; local PyTorch FADC-aligned implementation",
                }
                rows.append(row)
                write_csv(
                    per_seed_path,
                ...
                if result.diagnostics:
                    diag_path = out_dir / "figures" / "fadc_diagnostics" / f"{slug(method)}_seed{seed}_diagnostics.csv"
