...
MODEL_FACTORIES = {
    "FCN + FA-DCG V2c": FCNWithFADCGV2c,
    "U-Net + FA-DCG V2c": UNetWithFADCGV2c,
}

BENCHMARK_FACTORIES = {
    "FCN + FA-DCG V1.1": ("v1.1", FCNWithOptimizedFADCG),
    "FCN + FA-DCG V2c": ("v2c", FCNWithFADCGV2c),
    "U-Net + FA-DCG V1.1": ("v1.1", UNetWithOptimizedFADCG),
    "U-Net + FA-DCG V2c": ("v2c", UNetWithFADCGV2c),
}
...
def collect_diagnostics(model: nn.Module, images: torch.Tensor):
    modules = [module for module in model.modules() if isinstance(module, (FADCGV2a, FADCGV2b))]
    handles = []
...
def summarize(rows):
    ...
    for method in MODEL_FACTORIES:
        ...
        if delta_miou > 0:
            ...
            status = "v2c_comparable"
        elif delta_time > ref["time_ms"] * 1.5:
            status = "v2c_too_slow"
        else:
            status = "v2c_underperforming"
        summary.append(
            {
                "method": method,
                "implementation": "v2c",
...
def write_report(out_dir: Path, summary, speed_rows, args):
    ...
    report = [
        ...
        "## Recommendation",
        "",
        "- Promote V2c if it keeps FCN's V2a gain while improving or matching U-Net V2b.",
        "- If skip calibration hurts U-Net, keep V2a for FCN and V2b for U-Net as the final task_004 finding.",
        "",
        "## Run Settings",
        "",
    ...
    (out_dir / "task_004c_report.md").write_text("\n".join(report) + "\n", encoding="utf-8")


def main():
    args = parse_args()
    if args.output_root is None:
        args.output_root = str(REPO_ROOT / "exp" / "task_004c_fadcg_v2c_repro" / datetime.now().strftime("%Y%m%d_%H%M%S"))
    out_dir = Path(args.output_root)
    (out_dir / "logs").mkdir(parents=True, exist_ok=True)
    (out_dir / "checkpoints").mkdir(parents=True, exist_ok=True)
    (out_dir / "figures" / "fadcg_v2_diagnostics").mkdir(parents=True, exist_ok=True)
    write_environment(out_dir)
    test_output = run_cmd([sys.executable, "scripts/task_004c_test_fadcg_v2c.py"])
    write_audit(out_dir, test_output)
    (out_dir / "run_commands.sh").write_text(
        "#!/usr/bin/env bash\n"
        "set -euo pipefail\n"
        f"python scripts/task_004c_fadcg_v2c_repro.py --output-root {out_dir} "
        ...
        for method in args.models:
            for seed in args.seeds:
                ...
                row = {
                    ...
                    "notes": "validation split used; Selected Backbone-Aware Frequency Refinement",
                }
                rows.append(row)
                write_csv(
                    per_seed_path,
