...
from models.improved.fadc_srfg_hybrid import FADCSRFGHybrid, UNetWithFADCSRFGHybrid  # noqa: E402
from models.improved.unet_fadcg_v2d_ablation import UNetWithFADCGV2dAblation  # noqa: E402


BASELINE_TASK008 = REPO_ROOT / "exp" / "task_008_main_module_table" / "20260525_181115"
SEED = 2026
...
MODEL_SPECS = [
    {
        ...
        "variant": "cbam",
        "priority": "baseline",
        "factory": lambda **kwargs: UNetWithCBAM(**kwargs),
        "notes": "confirmation baseline: U-Net bottleneck CBAM under the task_009 protocol",
    },
    {
        "method": "U-Net + FADC-aligned",
        "variant": "fadc_aligned",
        "priority": "baseline",
        "factory": lambda **kwargs: unet_fadc_aligned(**kwargs),
        "notes": "confirmation baseline: local PyTorch FADC-aligned block under the task_009 protocol",
    },
    {
        "method": "U-Net + V2d-noLocalVar",
        "variant": "v2d_no_local_var",
        "priority": "A",
        "factory": lambda **kwargs: UNetWithFADCGV2dAblation(variant="no_local_var", **kwargs),
        "notes": "historical V2d ablation without local variance descriptor",
    },
    {
        "method": "U-Net + FADC-SR-high",
        "variant": "fadc_sr_high",
        "priority": "A",
        "factory": lambda **kwargs: UNetWithFADCSRFGHybrid(sr_mode="high", spatial_mode=None, **kwargs),
        "notes": "FADC-aligned main operator; SRFG reliability modulates high-frequency selected bands",
    },
    {
        "method": "U-Net + FADC-SARSpatial-noVar",
        "variant": "fadc_sar_spatial_no_var",
        "priority": "A",
        "factory": lambda **kwargs: UNetWithFADCSRFGHybrid(spatial_mode="sar", spatial_use_local_var=False, **kwargs),
        "notes": "FADC-aligned plus SAR-guided spatial mask without local variance",
    },
    {
        "method": "U-Net + FADC-SR-high + SARSpatial-noVar",
        "variant": "fadc_sr_high_sar_spatial_no_var",
        "priority": "A",
        "factory": lambda **kwargs: UNetWithFADCSRFGHybrid(
            sr_mode="high", spatial_mode="sar", spatial_use_local_var=False, **kwargs
        ),
        "notes": "combined FADC high-band reliability and SAR-guided spatial mask",
    },
    {
        "method": "U-Net + FADC-CBAM-SpatialOnly",
        "variant": "fadc_cbam_spatial_only",
        "priority": "A",
        "factory": lambda **kwargs: UNetWithFADCSRFGHybrid(spatial_mode="cbam", **kwargs),
        "notes": "control: FADC-aligned plus generic CBAM-style spatial-only mask",
    },
    {
        ...
        "notes": "V2d ablation without boundary, MSC, or local variance; keeps channel and speckle gates",
    },
    {
        "method": "U-Net + FADC-SR-branch",
        "variant": "fadc_sr_branch",
        "priority": "B",
        "factory": lambda **kwargs: UNetWithFADCSRFGHybrid(sr_mode="branch", spatial_mode=None, **kwargs),
        "notes": "FADC-aligned main operator; SRFG reliability modulates dilation branch responses",
    },
    {
        "method": "U-Net + FADC-SARSpatial-withVar",
        "variant": "fadc_sar_spatial_with_var",
        "priority": "B",
        "factory": lambda **kwargs: UNetWithFADCSRFGHybrid(
            spatial_mode="sar", spatial_use_local_var=True, **kwargs
        ),
        "notes": "FADC-aligned plus SAR-guided spatial mask with local variance descriptor",
    },
    {
        "method": "U-Net + FADC-SR-branch + SARSpatial-noVar",
        "variant": "fadc_sr_branch_sar_spatial_no_var",
        "priority": "B",
        "factory": lambda **kwargs: UNetWithFADCSRFGHybrid(
            sr_mode="branch", spatial_mode="sar", spatial_use_local_var=False, **kwargs
        ),
        "notes": "combined branch reliability and SAR-guided spatial mask",
    },
]
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--baseline-root", default=str(BASELINE_TASK008))
...
def load_task008_baselines(path: Path) -> Tuple[List[Dict[str, object]], List[Dict[str, object]]]:
    raw_rows = []
    comp_rows = []
                ...
                raw_rows.append(
                    {
                        ...
                        "seed": int(row["seed"]),
                        "val_miou": row.get("val_miou", row["miou"]),
                        "val_dice": row.get("val_dice", row["dice"]),
                        "test_miou": row.get("test_miou", row["miou"]),
                        "test_dice": row.get("test_dice", row["dice"]),
                        "best_epoch": row["best_epoch"],
                        "checkpoint_path": row["checkpoint_path"],
                        "status": "reused",
                        "notes": f"reused from task_008: {raw_path}",
                    }
                )
    if comp_path.exists():
        ...
        summary.append(
            {
                ...
                "fps": comp.get("fps", "NA"),
                "notes": "screening summary; reused rows imported from task_008 when noted",
            }
        )
    write_csv(
...
def extract_bottleneck(model: nn.Module, images: torch.Tensor) -> torch.Tensor | None:
    target = model
    if isinstance(model, UNetWithFADCSRFGHybrid):
        target = model
    elif hasattr(model, "light_fadc"):
        target = model
    else:
        return None
    skip_connections = []
...
def write_model_diagnostics(out_dir: Path, spec: Dict[str, object], model: nn.Module, loader: DataLoader, device: torch.device, seed: int) -> None:
    ...
    try:
        ...
        if bottleneck is not None and isinstance(getattr(model, "hybrid", None), FADCSRFGHybrid):
            diagnostics = model.hybrid.collect_input_diagnostics(bottleneck)
...
def write_implementation_audit(args: argparse.Namespace, out_dir: Path, specs: List[Dict[str, object]]) -> None:
    ...
    with (out_dir / "implementation_audit.md").open("w", encoding="utf-8") as handle:
        handle.write("# Task 009 Implementation Audit\n\n")
        for spec in specs:
            model = make_model(spec, device)
            ...
            if isinstance(getattr(model, "hybrid", None), FADCSRFGHybrid):
                handle.write("- hybrid initialization: frequency heads and optional reliability/spatial heads are zero-initialized; alpha starts at 0.1; spatial gamma starts at 0.0.\n")
            handle.write("- FADC-aligned is a local PyTorch implementation inspired by FADC, not the official implementation.\n\n")
            del model
            if device.type == "cuda":
                torch.cuda.empty_cache()
...
def write_report(
    ...
    with (out_dir / "task_009_report.md").open("w", encoding="utf-8") as handle:
        handle.write("# Task 009 FADC-SRFG Hybrid Report\n\n")
        ...
        handle.write("- Reused baselines: U-Net, U-Net + CBAM, U-Net + FADC-aligned, U-Net + SRFG from task_008 when available.\n")
        for spec in specs:
        ...
        if failures:
            ...
            handle.write("- None.\n")
        handle.write("\n## Summary\n\n")
        handle.write("| method | variant | test mIoU | test Dice | delta vs CBAM | params | MACs | latency ms | FPS |\n")
        handle.write("|---|---:|---:|---:|---:|---:|---:|---:|---:|\n")
        for row in summary:
            delta = float(row["test_miou_mean"]) - cbam_miou if not math.isnan(cbam_miou) else math.nan
            handle.write(
                f"| {row['method']} | {row['variant']} | {row['test_miou_mean']} | {row['test_dice_mean']} | "
                f"{delta:.6f} | {row['params_abs']} | {row['macs_abs']} | {row['latency_ms_mean']} | {row['fps']} |\n"
            )
        handle.write("\n## Decision Notes\n\n")
        ...
        if best:
            ...
            if delta >= 0.003:
                handle.write("- Decision rule: run multi-seed confirmation for this candidate.\n")
            elif delta > 0:
                handle.write("- Decision rule: improvement is positive but below 0.003 mIoU; treat as inconclusive until multi-seed confirmation.\n")
            else:
                handle.write("- Decision rule: no hybrid beats CBAM in seed-2026 screening.\n")
...
def main() -> None:
    ...
    out_dir = Path(args.output_root) if args.output_root else REPO_ROOT / "exp" / "task_009_fadc_srfg_hybrid" / timestamp
    ...
    baseline_raw, _ = load_task008_baselines(Path(args.baseline_root))
